...
@implementation CenterAdvertTVCell
{
    CALayer *_BVLayer;
}
...
- (IBAction)editBtnClick:(UIButton *)sender {
    ...
    EditAdvertVC * adVC = [[EditAdvertVC alloc]init];
    adVC.isEdit = YES;
    NSInteger index = self.bannerICV.currentItemIndex;
    adVC.adModel = self.centerModel.ad[index];
    [self.viewContoller.navigationController pushViewController:adVC animated:YES];
}
-(void)updateWithModel
{
    self.pageC.numberOfPages = self.centerModel.ad.count;
    if (self.centerModel.ad.count == 0) {
...
{
    return  self.centerModel.ad.count;
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view
{
    ...
    //    CGSize size =self.frame.size;
        ...
        CenterAdvertModel * imgModel = self.centerModel.ad[index];
