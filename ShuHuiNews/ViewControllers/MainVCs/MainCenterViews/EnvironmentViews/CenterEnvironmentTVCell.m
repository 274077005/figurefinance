...
@implementation CenterEnvironmentTVCell{
        //滚动视图的item数组
...
- (IBAction)editBtnClick:(UIButton *)sender {
    EditEnvironmentVC * enVC = [[EditEnvironmentVC alloc]init];
    NSInteger index = self.iCarouselV.currentItemIndex;
    enVC.enModel = self.centerModel.environment[index];
    enVC.isEdit = YES;
    [self.viewContoller.navigationController pushViewController:enVC animated:YES];
}
...
-(void)updateWithModel{
    ...
    if (self.centerModel.readEnv == NO) {
...
-(void)createMenuScrollView{
        ...
        if (self.centerModel.environment.count <=0) {
        ...
        //创建其他的市场按钮
        float menuWidth = 0.0;
        for (int i = 0;i < self.centerModel.environment.count; i++) {
            NSInteger itemWidth = 60;
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            
            CenterEnvironmentModel * menuMdeol = self.centerModel.environment[i];
...
- (NSInteger)numberOfItemsInCarousel:(iCarousel *)carousel{
    ...
    return self.centerModel.environment.count;
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view{
    ...
    CenterEnvironmentModel * enModel = self.centerModel.environment[index];
