...
@implementation CenterTagInfoTVCell
{
    CALayer * _BVLayer;
}
...
- (IBAction)editBtnClick:(UIButton *)sender {
    CenterTagVC * tagVC = [[CenterTagVC alloc]init];
    if ([[UserInfo share].type isEqualToString:@"1"]) {
       tagVC.industryType = @"3";
    }else{
        tagVC.industryType = @"2";
    }
    
    tagVC.tagModel = _centerModel.attestation_tag;
    [self.viewContoller.navigationController pushViewController:tagVC animated:YES];
...
-(void)setUpScrollV
{
    ...
    if (self.centerModel.attestation_tag.attestation_tag.length < 1) {
        return;
    }
    NSArray * tagArr = [self.centerModel.attestation_tag.attestation_tag componentsSeparatedByString:@"|"];
