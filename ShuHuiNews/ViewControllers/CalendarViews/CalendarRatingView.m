...
@implementation CalendarRatingView
...
- (void)setUpContentView
{
    ...
    [self.grayStarView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self.foreStarView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    for (int i = 0; i < starNumber; i++) {
        ...
        UIImage * foreImg;
        if (_threeStar) {
            foreImg = [UIImage imageNamed:@"starBlue"];
        }else{
            foreImg = [UIImage imageNamed:@"starBlack"];
        }
