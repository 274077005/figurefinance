...
//  github地址：https://github.com/WellsYC/BubbleView
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _separatorColor = RGBCOLOR(239, 239, 239);
    }
    return self;
}
...
@property (copy, nonatomic)void (^handler)(BubbleAction *);
@end
@implementation BubbleAction
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(BubbleAction *))handler{
    BubbleAction *action = [[BubbleAction alloc] initWithTitle:title image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(BubbleAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
@property(nonatomic,copy) NSArray<BubbleAction *>   *actions;
...
+ (instancetype)menuWithActions:(NSArray<BubbleAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    NSAssert(width>0.0f, @"width要大于0");
    BubbleView *menu = [[BubbleView alloc] initWithActions:actions width:width atPoint:point];
    return menu;
}
+ (instancetype)menuWithActions:(NSArray<BubbleAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    NSAssert(width>0.0f, @"width要大于0");
    NSAssert([view isKindOfClass:[UIView class]]||[view isKindOfClass:[UIBarButtonItem class]], @"relyonView必须是UIView或UIBarButtonItem");
    BubbleView *menu = [[BubbleView alloc] initWithActions:actions width:width relyonView:view];
    return menu;
}

- (instancetype)initWithActions:(NSArray<BubbleAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray<BubbleAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (void)defaultConfiguration{
    ...
    _textFont = [UIFont boldSystemFontOfSize:14.0f];
    _offset = 0.0f;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BubbleAction *action = _actions[indexPath.row];
